...
# Download NG from EIA website
  ...
  Date <- NG_Res$Date[205:541]
  
# Filter 1990 to 2017
  NG_Res <- NG_Res %>%
    filter(str_detect(Date, "^199|^20")) # filter years with 199x and 20xx
...
##### Load GDP (chained 2012 dollar) #####
    ...
    saveRDS(df, here("data","state_monthly_GDP.rds"))
...
##### Interpolate Residential Population #####
    ...
    saveRDS(df, here("data","state_monthly_pop.rds"))
...
##### Build State Profiles #####
  ...
  # Load Population and eliminate DC, Hawaii and Alaska
    Pop.m <- readRDS(here("data", "state_monthly_pop.rds"))[, -c(2, 9, 12)]
  # Load GDP
    GDP.m <- readRDS(here("data", "state_monthly_GDP.rds"))[, -c(2, 9, 12)]
  # Load CDD
  ...
  for (i in 1:48){
     ...
     statename <- colnames(Pop.m, prefix = "col")[i]
    state <- tibble(
      "Date" = Date[1:336],
